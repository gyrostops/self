...
export default async function Message(hisoka, m, chatUpdate) {
    try {
        ...
        const thumb = 'https://telegra.ph/file/24352efcd655a260776b5.jpg'
...
`.trim()
            ...
            case "ai": case "chatgpt": case "openai": {
                if (!m.text) throw `Mau Nanya Apa?`
    let res = await fetch(`https://api-xgtib.cyclic.app/api/aitekno/chatgpt?query=${m.text}&apikey=Xgtib`)
    let anu = await res.json()

let content = anu.data.result
